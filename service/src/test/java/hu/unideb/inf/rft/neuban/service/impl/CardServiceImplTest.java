...
public class CardServiceImplTest {
	...
	// private final UserDto firstUserDto =
	// UserDto.builder().id(FIRST_USER_ID).userName(USERNAME).password(PASSWORD_HASH)
	// .build();
	//
	// private final UserDto secondUserDto =
	// UserDto.builder().id(SECOND_USER_ID).userName(USERNAME)
	// .password(PASSWORD_HASH).build();
	//
	// private final UserDto thirdUserDto =
	// UserDto.builder().id(THIRD_USER_ID).userName(USERNAME).password(PASSWORD_HASH)
	// .build();
	//
	// private final UserDto fourthUserDto =
	// UserDto.builder().id(FOURTH_USER_ID).userName(USERNAME)
	// .password(PASSWORD_HASH).build();
	...
	public void getAllByTitleContainingShouldReturnEmptyListWhenParamSearchStringDoesNotFindAnyCard()
		...
		given(modelMapper.map(cardRepository.findByTitleContaining(searchString), listType))
	...
	public void getAllByTitleContainingShouldReturnCardDtoListWhenParamSearchStringDoesFindCardWithContainingTitle()
			throws CardNotFoundException {
		// Given
		given(modelMapper.map(cardRepository.findByTitleContaining(CARD_TITLE), listType))
